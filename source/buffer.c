...
// Grow the ring buffer, returning the slot you want to write to as the "grown"
// item
...
// Fill up ring buffer as much as safely possible with pre-parsed full lines,
// ready for drawing at any time.
char *scan_lines(struct LineRingBuffer *buffer, char *drawdata,
                 char *drawdata_end, const u16 page) {
  char *scandata = drawdata;
  char *stroke_pointer = NULL;
  u32 scan_remaining = MAX_DRAWDATA_SCAN; // Some arbitrarily high number. This
                                          // may require tweaking!
  while (scan_remaining > 0 && scandata < drawdata_end &&
         buffer->capacity - 1 - lineringbuffer_size(buffer) >
             MAX_STROKE_LINES) {
    // Find and parse the next stroke in our page.
    scan_remaining = MAX_DRAWDATA_SCAN - (scandata - drawdata);
    scandata = datamem_scanstroke(scandata, drawdata_end, scan_remaining, page,
                                  &stroke_pointer);
