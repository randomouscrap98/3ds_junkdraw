...
void initialize_easy_menu_state(struct EasyMenuState *state) {
  ...
  state->_title_height =
      state->_has_title ? (1 + char_occurrences(state->title, '\n')) : 0;
  ...
  while (state->_menu_num < MAX_MENU_ITEMS &&
         *state->_menu_strings[state->_menu_num] != 0) {
    state->_menu_strings[state->_menu_num + 1] =
        state->_menu_strings[state->_menu_num] +
  ...
  if (state->max_height < min_totalheight) {
    LOGDBG("MENU TOO SMALL, INCREASING FROM %d TO %d", state->max_height,
           min_totalheight);
...
void clear_easy_menu(struct EasyMenuState *state) {
  printf("\x1b[0m\x1b[%d;1H%*s", state->top,
         (state->_title_height + state->_display_items + 2) * 50, "");
}
...
void draw_easy_menu(struct EasyMenuState *state) {
  if (state->_has_title) {
    printf("\x1b[0m%s\x1b[%d;1H %s\n", state->home_rejected ? "\x1b[31m" : "",
           state->top,
           state->title); // Still keeping that space for some reason.
  }
  ...
  printf("\x1b[0m\x1b[%d;3H%s", state->top + state->_title_height,
         (state->menu_ofs > 0) ? "..." : "   ");
  printf("\x1b[%d;3H%s",
         state->top + state->_title_height + state->_display_items + 1,
...
bool modify_easy_menu_state(struct EasyMenuState *state, u32 kDown,
                            u32 kRepeat) {
  ...
  if (kRepeat & KEY_UP) {
    state->selected_index =
        (state->selected_index - 1 + state->_menu_num) % state->_menu_num;
...
void draw_enter_text(struct EnterTextState *state) {
  ...
  for (u8 i = 0; i < state->entry_max; i++) {
    ...
    printf("\x1b[0m\x1b[%d;%dH%c\x1b[%d;%dH%c\x1b[%d;%dH%c", y, x,
           state->text[i],                 // The char data
           y - 1, x, selected ? '-' : ' ', // The up/down
           y + 1, x, selected ? '-' : ' ');
  }
}

bool modify_enter_text_state(struct EnterTextState *state, u32 kDown,
                             u32 kRepeat) {
  u8 current_index =
      (strchr(state->charset, state->text[state->selected_index]) -
       state->charset);
  ...
  if (kRepeat & KEY_RIGHT && state->selected_index < state->entry_max - 1)
    state->selected_index++;
  if (kRepeat & KEY_LEFT && state->selected_index > 0)
    state->selected_index--;
  if (kRepeat & KEY_UP) {
  ...
  if (kRepeat & KEY_DOWN) {
    state->text[state->selected_index] =
        state->charset[(current_index - 1 + state->_charset_count) %
                       state->_charset_count];
  }

  return false;
}

// Allows user to submit a fixed length text using the dpad. HIGHLY limited
// characters
bool enter_text_fixed(const char *title, u8 top, char *container,
                      u8 fixed_length, bool clear, u32 exit_buttons) {
